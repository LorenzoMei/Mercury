...
CREATE TABLE `utenteregistrato` (
  `idUtenteRegistrato` int NOT NULL AUTO_INCREMENT,
  `email` varchar(45) NOT NULL,
  `tipo` varchar(45) DEFAULT NULL,
  `cadenza` varchar(45) DEFAULT NULL,
  `zonaFk` int DEFAULT NULL,
  `data` varchar(45) NOT NULL,
  PRIMARY KEY (`idUtenteRegistrato`),
  UNIQUE KEY `email_UNIQUE` (`email`),
  KEY `zonaFk_idx` (`zonaFk`),
  CONSTRAINT `zonaFk` FOREIGN KEY (`zonaFk`) REFERENCES `zona` (`idZona`) ON DELETE CASCADE ON UPDATE CASCADE
